...
@router.get("/{site_id}/us", response_class=RedirectResponse)
async def view_site_us_redirect(site_id: UUID):
    """
    Reindirizza dalla vecchia URL /view/{site_id}/us 
    alla nuova URL corretta /us?site={site_id}
    """
    logger.info(f"Redirecting from /view/{site_id}/us to /us?site={site_id}")
    return RedirectResponse(
        url=f"/us?site={site_id}", 
        status_code=302
    )

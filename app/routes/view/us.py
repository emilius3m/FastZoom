...
@us_view_router.get("/{site_id}/harris-matrix", response_class=HTMLResponse)
async def harris_matrix_view(
    request: Request,
    site_id: UUID,
    current_user_id: UUID = Depends(get_current_user_id_with_blacklist),
    user_sites: List[Dict[str, Any]] = Depends(get_current_user_sites_with_blacklist),
    ...
    """Visualizza Harris Matrix per il sito"""
    ...
    # Prepara context per il template usando la funzione helper unificata
    context = await get_base_template_context(
        request, current_user_id, user_sites, db, site, permission, "harris-matrix"
    )
    context.update({
        "site_id": str(site_id),
        "user_role": permission.permission_level if permission else "none"

...
async def v1_atomic_save_harris_matrix(
    ...
    try:
        ...
        # FastAPI's get_async_session already provides an implicit transaction
        # No need for explicit db.begin() - this fixes "A transaction is already begun" error
        try:
            operation_results = {
            ...
            if request.existing_units_updates and request.existing_units_updates.updates:
                ...
                operation_results["update_units"] = {
                    "success": True,
                    "message": f"Updated sequenzafisica for {update_result['updated_count']} units",
                    "data": update_result
                }
            ...
            if request.layout_positions and request.layout_positions.positions:
                logger.info(f"STEP 3: Saving layout for {len(request.layout_positions.positions)} positions")
            ...
            # Re-raise HTTP exceptions - rollback transaction
            await db.rollback()
            raise
        except Exception as e:

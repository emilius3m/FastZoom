...
async def get_user_id_from_request(request: Request, db: AsyncSession) -> Optional[UUID]:
    """
    Try multiple authentication methods: session -> JWT header -> JWT cookie.
    Returns user_id if authenticated, None otherwise.
    """
    # 1. Session authentication
    if request.session.get("user_id"):
        try:
            return UUID(request.session.get("user_id"))
        except (ValueError, TypeError):
            pass
    
    # 2. JWT header authentication
    auth_header = request.headers.get("Authorization")
    if auth_header and auth_header.startswith("Bearer "):
        try:
            from app.core.security import SecurityService
            payload = await SecurityService.verify_token(auth_header.replace("Bearer ", ""), db)
    ...
    if access_token:
        try:
            from app.core.security import SecurityService
            payload = await SecurityService.verify_token(access_token.replace("Bearer ", ""), db)
            return UUID(payload.get("sub"))
        except Exception:
            pass
...
async def get_deep_zoom_tile(
    ...
    try:
        ...
        logger.error(f"Tile retrieval error: {site_id}/{photo_id}/{level}/{x}_{y} - {e}")
        raise
...
async def get_public_deep_zoom_tile(
    ...
    # Get user from session, JWT header, or JWT cookie
    current_user_id = await get_user_id_from_request(request, db)
    ...
    photo = await db.execute(
        select(Photo).where(and_(Photo.id == str(photo_id), Photo.site_id == str(site_id)))
    )
    ...
    try:
        ...
        logger.error(f"Public tile error: {site_id}/{photo_id}/{level}/{x}_{y} - {e}")
        raise HTTPException(status_code=500, detail="Errore caricamento tile")

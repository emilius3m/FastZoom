...
class GeographicMap(Base):
    """
    Modello per mappe geografiche associate ai siti
    Include configurazioni cartografiche, layer, bookmark
    """
    ...
    # Configurazione view
    center_lat = Column(String(20), nullable=True)  # Centro mappa
    center_lng = Column(String(20), nullable=True)
    zoom_level = Column(Integer, default=15)  # Livello zoom default

    # Configurazioni layer
    base_layer = Column(String(50), default='osm')  # osm, satellite, terrain
    overlay_layers = Column(JSON, default=list)  # Layer aggiuntivi

    # Dati GIS
    boundaries = Column(JSON, nullable=True)  # Poligono confini sito
    points_of_interest = Column(JSON, default=list)  # POI sul sito
    measurement_data = Column(JSON, default=dict)  # Misurazioni

    # Configurazioni
    is_default = Column(Boolean, default=False)  # Mappa di default per sito
    is_public = Column(Boolean, default=False)  # Visibilità pubblica
    ...
    def __repr__(self):
        return f"<GeographicMap(name={self.name}, site={self.site.name if self.site else 'N/A'})>"

    @property
    def center_coordinates(self) -> Optional[Dict[str, float]]:
        """Centro mappa come coordinate"""
        if self.center_lat and self.center_lng:
            try:
                return {
                    'lat': float(self.center_lat),
                    'lng': float(self.center_lng)
                }
            except (ValueError, TypeError):
                return None
        return None

    def add_poi(self, name: str, lat: float, lng: float, description: str = None):
        """Aggiunge punto di interesse"""
        pois = self.points_of_interest or []
        poi = {
            'name': name,
            'lat': lat,
            'lng': lng,
    ...
    def set_boundaries(self, coordinates: List[List[float]]):
        """Imposta confini sito come poligono"""
        self.boundaries = {
            'type': 'Polygon',

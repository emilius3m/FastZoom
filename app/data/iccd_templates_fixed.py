"""Template JSON per schemi ICCD 3.00 ufficiali - Catalogazione Archeologica."""

from datetime import datetime
from typing import Dict, Any


def get_iccd_ra_300_template() -> Dict[str, Any]:
    """Template per scheda RA 3.00 - Reperto Archeologico secondo standard ICCD ufficiale."""
    
    return {
        "id": "iccd_ra_300",
        "name": "Scheda RA 3.00 - Reperti Archeologici ICCD",
        "category": "artifact",
        "icon": "🏺",
        "version": "3.00",
        "standard": "ICCD_MiC_2021",
        "schema": {
            "$schema": "https://json-schema.org/draft/2020-12/schema",
            "type": "object",
            "title": "SCHEDA RA 3.00 - REPERTI ARCHEOLOGICI",
            "description": "Standard ICCD per la catalogazione dei reperti archeologici mobili",
            
            "properties": {
                "CD": {
                    "type": "object",
                    "title": "CODICI",
                    "description": "Paragrafo obbligatorio con codici identificativi",
                    "properties": {
                        "TSK": {
                            "type": "string",
                            "title": "Tipo scheda",
                            "const": "RA",
                            "description": "Tipologia scheda: RA = Reperto Archeologico"
                        },
                        "LIR": {
                            "type": "string",
                            "title": "Livello ricerca", 
                            "enum": ["P", "C", "A"],
                            "enumNames": ["Precatalogazione", "Catalogazione", "Approfondimento"],
                            "default": "C",
                            "description": "P=Precatalogazione, C=Catalogazione, A=Approfondimento"
                        },
                        "NCT": {
                            "type": "object",
                            "title": "CODICE UNIVOCO",
                            "description": "Codice identificativo univoco nazionale",
                            "properties": {
                                "NCTR": {
                                    "type": "string", 
                                    "title": "Codice Regione",
                                    "pattern": "^[0-9]{2}$",
                                    "description": "Codice ISTAT regione (12=Lazio)"
                                },
                                "NCTN": {
                                    "type": "string",
                                    "title": "Numero catalogo generale", 
                                    "pattern": "^[0-9]{8}$",
                                    "description": "8 cifre - numero progressivo nazionale"
                                },
                                "NCTS": {
                                    "type": "string",
                                    "title": "Suffisso",
                                    "pattern": "^[A-Z]{0,2}$",
                                    "description": "Massimo 2 lettere maiuscole"
                                }
                            },
                            "required": ["NCTR", "NCTN"],
                            "additionalProperties": false
                        },
                        "ESC": {
                            "type": "string",
                            "title": "Ente schedatore",
                            "description": "Codice ente responsabile redazione scheda",
                            "examples": ["SSABAP-RM", "SABAP-RM", "UnivRoma1"]
                        },
                        "ECP": {
                            "type": "string", 
                            "title": "Ente competente",
                            "description": "Codice ente MiC competente per tutela"
                        }
                    },
                    "required": ["TSK", "LIR", "NCT", "ESC"],
                    "additionalProperties": false
                },

                "OG": {
                    "type": "object",
                    "title": "OGGETTO", 
                    "description": "Paragrafo obbligatorio - identificazione del bene",
                    "properties": {
                        "OGT": {
                            "type": "object",
                            "title": "OGGETTO",
                            "properties": {
                                "OGTD": {
                                    "type": "string",
                                    "title": "Definizione",
                                    "description": "Termine che identifica il bene - da Thesaurus ICCD",
                                    "examples": ["coppa", "anfora", "lucerna", "fibula", "moneta", "balsamario", "skyphos"]
                                },
                                "OGTT": {
                                    "type": "string",
                                    "title": "Tipologia", 
                                    "description": "Specificazione tipologica del bene"
                                },
                                "OGTN": {
                                    "type": "string",
                                    "title": "Denominazione",
                                    "description": "Nome proprio se conosciuto"
                                }
                            },
                            "required": ["OGTD"],
                            "additionalProperties": false
                        },
                        "CLS": {
                            "type": "object",
                            "title": "CLASSIFICAZIONE",
                            "properties": {
                                "CLSC": {
                                    "type": "string",
                                    "title": "Categoria",
                                    "enum": [
                                        "ABBIGLIAMENTO E ORNAMENTI PERSONALI",
                                        "ARREDI", 
                                        "EDILIZIA",
                                        "MEZZI DI TRASPORTO",
                                        "PITTURA",
                                        "REPERTI ARCHEOBOTANICI", 
                                        "REPERTI ARCHEOZOOLOGICI",
                                        "SCULTURA",
                                        "STRUMENTI-UTENSILI-OGGETTI D'USO"
                                    ]
                                },
                                "CLSS": {
                                    "type": "string",
                                    "title": "Classe",
                                    "description": "Classe del reperto"
                                },
                                "CLSP": {
                                    "type": "string", 
                                    "title": "Produzione",
                                    "description": "Produzione quando identificabile"
                                }
                            }
                        }
                    },
                    "required": ["OGT"],
                    "additionalProperties": false
                },

                "LC": {
                    "type": "object",
                    "title": "LOCALIZZAZIONE GEOGRAFICO-AMMINISTRATIVA",
                    "description": "Paragrafo obbligatorio - dove si trova il bene",
                    "properties": {
                        "PVC": {
                            "type": "object",
                            "title": "LOCALIZZAZIONE GEOGRAFICO-AMMINISTRATIVA",
                            "properties": {
                                "PVCS": {
                                    "type": "string",
                                    "title": "Stato",
                                    "default": "Italia"
                                },
                                "PVCR": {
                                    "type": "string",
                                    "title": "Regione", 
                                    "description": "Da codici ISTAT",
                                    "default": "Lazio"
                                },
                                "PVCP": {
                                    "type": "string",
                                    "title": "Provincia",
                                    "description": "Sigla provincia",
                                    "default": "RM"
                                },
                                "PVCC": {
                                    "type": "string", 
                                    "title": "Comune",
                                    "description": "Nome comune",
                                    "default": "Roma"
                                },
                                "PVCL": {
                                    "type": "string",
                                    "title": "Località",
                                    "description": "Frazione, località"
                                }
                            },
                            "required": ["PVCS", "PVCR", "PVCP", "PVCC"],
                            "additionalProperties": false
                        },
                        "PVL": {
                            "type": "object",
                            "title": "LOCALIZZAZIONE SPECIFICA", 
                            "properties": {
                                "PVLN": {
                                    "type": "string",
                                    "title": "Denominazione",
                                    "description": "Nome del sito/area",
                                    "default": "Domus Flavia",
                                    "examples": ["Domus Flavia", "Palatino", "Foro Romano"]
                                },
                                "PVLI": {
                                    "type": "string",
                                    "title": "Indirizzo",
                                    "description": "Via, piazza, località"
                                },
                                "PVLE": {
                                    "type": "string",
                                    "title": "Ente di appartenenza"
                                }
                            }
                        },
                        "LDC": {
                            "type": "object", 
                            "title": "COLLOCAZIONE SPECIFICA",
                            "description": "Contenitori fisici e giuridici",
                            "properties": {
                                "LDCN": {
                                    "type": "string",
                                    "title": "Denominazione raccolta",
                                    "description": "Nome collezione/deposito"
                                },
                                "LDCU": {
                                    "type": "string",
                                    "title": "Ubicazione",
                                    "description": "Ubicazione specifica"
                                },
                                "LDCQ": {
                                    "type": "string",
                                    "title": "Qualificazione",
                                    "enum": ["contenitore fisico", "contenitore giuridico"]
                                },
                                "LDCS": {
                                    "type": "string",
                                    "title": "Specifiche",
                                    "description": "Dettagli aggiuntivi ubicazione"
                                }
                            }
                        }
                    },
                    "required": ["PVC"],
                    "additionalProperties": false
                },

                "DT": {
                    "type": "object",
                    "title": "CRONOLOGIA",
                    "description": "Paragrafo obbligatorio - datazione del bene",
                    "properties": {
                        "DTZ": {
                            "type": "array",
                            "title": "CRONOLOGIA GENERICA",
                            "items": {
                                "type": "object", 
                                "properties": {
                                    "DTZG": {
                                        "type": "string",
                                        "title": "Fascia cronologica",
                                        "examples": ["I secolo d.C.", "età augustea", "epoca flavia", "II-III secolo d.C."]
                                    },
                                    "DTZS": {
                                        "type": "string",
                                        "title": "Validità",
                                        "enum": ["ca.", "?", "ante", "post", "non ante", "non post"]
                                    }
                                },
                                "required": ["DTZG"]
                            },
                            "minItems": 1
                        },
                        "DTS": {
                            "type": "array",
                            "title": "CRONOLOGIA SPECIFICA",
                            "items": {
                                "type": "object",
                                "properties": {
                                    "DTSI": {
                                        "type": "integer", 
                                        "title": "Da (anno)",
                                        "minimum": -753,
                                        "maximum": 2100,
                                        "description": "Anno iniziale"
                                    },
                                    "DTSF": {
                                        "type": "integer",
                                        "title": "A (anno)", 
                                        "minimum": -753,
                                        "maximum": 2100,
                                        "description": "Anno finale"
                                    },
                                    "DTSV": {
                                        "type": "string",
                                        "title": "Validità",
                                        "enum": ["ca.", "?", "ante", "post"]
                                    }
                                }
                            }
                        },
                        "DTM": {
                            "type": "string",
                            "title": "Motivazione cronologia",
                            "enum": ["analisi stilistica", "dati stratigrafici", "confronti", "dati epigrafici", "analisi archeometriche", "documentazione d'archivio"],
                            "description": "Fonte dell'attribuzione cronologica"
                        }
                    },
                    "anyOf": [
                        { "required": ["DTZ"] },
                        { "required": ["DTS"] }
                    ],
                    "additionalProperties": false
                },

                "MT": {
                    "type": "object",
                    "title": "DATI TECNICI",
                    "description": "Paragrafo obbligatorio - materia, tecnica, misure",
                    "properties": {
                        "MTC": {
                            "type": "object",
                            "title": "MATERIA E TECNICA",
                            "properties": {
                                "MTCM": {
                                    "type": "array", 
                                    "title": "Materia",
                                    "description": "Materiali costitutivi",
                                    "items": {
                                        "type": "string",
                                        "enum": [
                                            "ceramica", "terracotta", "argilla",
                                            "bronzo", "ferro", "piombo", "rame", "oro", "argento",
                                            "marmo", "travertino", "tufo", "pietra calcarea", 
                                            "legno", "osso", "corno", "avorio",
                                            "vetro", "pasta vitrea", "ambra",
                                            "tessuto", "cuoio", "pelle"
                                        ]
                                    },
                                    "minItems": 1,
                                    "uniqueItems": true
                                },
                                "MTCT": {
                                    "type": "array",
                                    "title": "Tecnica",
                                    "description": "Tecniche di lavorazione",
                                    "items": {
                                        "type": "string", 
                                        "examples": ["tornio", "modellato a mano", "matrice", "fusione", "martellato", "inciso", "dipinto"]
                                    }
                                }
                            },
                            "required": ["MTCM"],
                            "additionalProperties": false
                        },
                        "MIS": {
                            "type": "object",
                            "title": "MISURE", 
                            "description": "Almeno una misura obbligatoria",
                            "properties": {
                                "MISA": {
                                    "type": "number",
                                    "title": "Altezza",
                                    "description": "Altezza in cm",
                                    "minimum": 0,
                                    "multipleOf": 0.01
                                },
                                "MISL": {
                                    "type": "number",
                                    "title": "Larghezza", 
                                    "description": "Larghezza in cm",
                                    "minimum": 0,
                                    "multipleOf": 0.01
                                },
                                "MISP": {
                                    "type": "number",
                                    "title": "Profondità",
                                    "description": "Profondità in cm", 
                                    "minimum": 0,
                                    "multipleOf": 0.01
                                },
                                "MISD": {
                                    "type": "number",
                                    "title": "Diametro",
                                    "description": "Diametro in cm",
                                    "minimum": 0,
                                    "multipleOf": 0.01
                                },
                                "MISE": {
                                    "type": "number",
                                    "title": "Spessore",
                                    "description": "Spessore in cm",
                                    "minimum": 0,
                                    "multipleOf": 0.01
                                },
                                "MISU": {
                                    "type": "string",
                                    "title": "Unità di misura",
                                    "default": "cm",
                                    "enum": ["mm", "cm", "m"]
                                }
                            },
                            "anyOf": [
                                { "required": ["MISA"] },
                                { "required": ["MISL"] }, 
                                { "required": ["MISP"] },
                                { "required": ["MISD"] },
                                { "required": ["MISE"] }
                            ],
                            "additionalProperties": false
                        }
                    },
                    "required": ["MTC", "MIS"],
                    "additionalProperties": false
                },

                "DA": {
                    "type": "object",
                    "title": "DATI ANALITICI",
                    "description": "Paragrafo obbligatorio - descrizione del bene",
                    "properties": {
                        "DES": {
                            "type": "object",
                            "title": "DESCRIZIONE",
                            "properties": {
                                "DESO": {
                                    "type": "string", 
                                    "title": "Descrizione oggetto",
                                    "description": "Descrizione dettagliata del reperto",
                                    "minLength": 20
                                }
                            },
                            "required": ["DESO"],
                            "additionalProperties": false
                        },
                        "ISR": {
                            "type": "array",
                            "title": "ISCRIZIONI",
                            "items": {
                                "type": "object",
                                "properties": {
                                    "ISRC": {
                                        "type": "string",
                                        "title": "Classe",
                                        "enum": ["votiva", "funeraria", "onoraria", "sacra", "profana", "instrumentum domesticum", "numerale", "di possesso"]
                                    },
                                    "ISRI": {
                                        "type": "string",
                                        "title": "Trascrizione",
                                        "description": "Testo dell'iscrizione"
                                    },
                                    "ISRL": {
                                        "type": "string",
                                        "title": "Lingua",
                                        "enum": ["latino", "greco", "etrusco", "osco", "umbro", "messapico"]
                                    },
                                    "ISRT": {
                                        "type": "string", 
                                        "title": "Tecnica scrittura",
                                        "enum": ["graffito", "dipinto", "inciso", "impresso", "rilievo"]
                                    }
                                }
                            }
                        },
                        "NSC": {
                            "type": "string",
                            "title": "Notizie storico-critiche",
                            "description": "Campo a testo libero per notizie aggiuntive"
                        }
                    },
                    "required": ["DES"],
                    "additionalProperties": false
                },

                "CO": {
                    "type": "object",
                    "title": "CONSERVAZIONE",
                    "description": "Stato di conservazione del bene",
                    "properties": {
                        "STC": {
                            "type": "object",
                            "title": "STATO DI CONSERVAZIONE",
                            "properties": {
                                "STCC": {
                                    "type": "string",
                                    "title": "Stato di conservazione", 
                                    "enum": ["ottimo", "buono", "discreto", "mediocre", "cattivo", "pessimo"],
                                    "enumNames": ["Ottimo", "Buono", "Discreto", "Mediocre", "Cattivo", "Pessimo"]
                                },
                                "STCS": {
                                    "type": "string",
                                    "title": "Indicazioni specifiche",
                                    "description": "Descrizione dettagliata stato conservativo"
                                }
                            },
                            "required": ["STCC"]
                        }
                    }
                },

                "AD": {
                    "type": "object",
                    "title": "ACCESSO AI DATI",
                    "description": "Paragrafo obbligatorio - profilo accesso",
                    "properties": {
                        "ADSP": {
                            "type": "string",
                            "title": "Profilo di accesso",
                            "enum": ["1", "2", "3"],
                            "enumNames": ["Libero", "Riservato enti", "Riservato"],
                            "default": "1",
                            "description": "1=libero, 2=riservato enti, 3=riservato"
                        }
                    },
                    "required": ["ADSP"],
                    "additionalProperties": false
                },

                "CM": {
                    "type": "object",
                    "title": "COMPILAZIONE", 
                    "description": "Paragrafo obbligatorio - dati di compilazione",
                    "properties": {
                        "CMP": {
                            "type": "object",
                            "title": "COMPILAZIONE",
                            "properties": {
                                "CMPD": {
                                    "type": "string",
                                    "title": "Data",
                                    "pattern": "^[0-9]{4}$",
                                    "description": "Anno di compilazione (YYYY)"
                                },
                                "CMPN": {
                                    "type": "string", 
                                    "title": "Nome compilatore",
                                    "description": "Chi ha compilato la scheda"
                                }
                            },
                            "required": ["CMPD", "CMPN"]
                        },
                        "RSR": {
                            "type": "string",
                            "title": "Responsabile scientifico",
                            "description": "Responsabile attività catalogazione"
                        },
                        "FUR": {
                            "type": "string",
                            "title": "Funzionario responsabile",
                            "description": "Funzionario responsabile tutela"
                        }
                    },
                    "required": ["CMP"],
                    "additionalProperties": false
                }
            },

            "required": ["CD", "OG", "LC", "DT", "MT", "DA", "AD", "CM"],
            "additionalProperties": false
        },
        
        "ui_schema": {
            "CD": {
                "ui:order": ["TSK", "LIR", "NCT", "ESC", "ECP"],
                "TSK": { "ui:readonly": true },
                "NCT": {
                    "ui:order": ["NCTR", "NCTN", "NCTS"],
                    "NCTR": { "ui:help": "Codice ISTAT regione (12 per Lazio)" },
                    "NCTN": { "ui:help": "8 cifre - generato automaticamente" }
                }
            },
            "OG": {
                "ui:order": ["OGT", "CLS"],
                "OGT": {
                    "ui:order": ["OGTD", "OGTT", "OGTN"],
                    "OGTD": {
                        "ui:help": "Utilizzare termini dal Thesaurus ICCD"
                    }
                },
                "CLS": {
                    "CLSC": {
                        "ui:help": "Scegliere dalla classificazione ICCD"
                    }
                }
            },
            "LC": {
                "ui:order": ["PVC", "PVL", "LDC"]
            },
            "MT": {
                "ui:order": ["MTC", "MIS"],
                "MTC": {
                    "MTCM": {
                        "ui:widget": "checkboxes"
                    },
                    "MTCT": {
                        "ui:widget": "checkboxes"
                    }
                }
            },
            "DA": {
                "DES": {
                    "DESO": {
                        "ui:widget": "textarea",
                        "ui:options": { "rows": 5 }
                    }
                },
                "NSC": {
                    "ui:widget": "textarea",
                    "ui:options": { "rows": 3 }
                }
            },
            "CM": {
                "CMP": {
                    "CMPN": {
                        "ui:help": "Nome e cognome del catalogatore"
                    }
                }
            }
        }
    }


# Mantieni le altre funzioni esistenti per compatibilità
def get_iccd_ra_template() -> Dict[str, Any]:
    """Wrapper per compatibilità - usa schema RA 3.00."""
    return get_iccd_ra_300_template()

def get_iccd_ca_template() -> Dict[str, Any]:
    """Template semplificato per CA."""
    return {
        "id": "iccd_ca_template",
        "name": "Scheda CA - Complesso Archeologico ICCD",
        "category": "architecture",
        "icon": "🏛️",
        "version": "3.00",
        "standard": "ICCD_MiC_2021"
    }

def get_iccd_si_template() -> Dict[str, Any]:
    """Template semplificato per SI."""
    return {
        "id": "iccd_si_template", 
        "name": "Scheda SI - Sito Archeologico ICCD",
        "category": "site",
        "icon": "🗺️",
        "version": "3.00",
        "standard": "ICCD_MiC_2021"
    }

# Dizionario con tutti i template disponibili
ICCD_TEMPLATES = {
    "RA": get_iccd_ra_300_template(),
    "CA": get_iccd_ca_template(), 
    "SI": get_iccd_si_template()
}

def get_template_by_type(schema_type: str) -> Dict[str, Any]:
    """Ottieni template ICCD per tipo schema."""
    return ICCD_TEMPLATES.get(schema_type, {})

def get_all_templates() -> Dict[str, Dict[str, Any]]:
    """Ottieni tutti i template ICCD disponibili."""
    return ICCD_TEMPLATES

def generate_default_iccd_data_ra300(schema_type: str, site_name: str = "Domus Flavia") -> Dict[str, Any]:
    """Genera dati ICCD di default per schema RA 3.00 ufficiale."""
    
    now = datetime.now()
    year = now.year % 100
    sequence = now.microsecond % 1000000
    nct_number = f"{year:02d}{sequence:06d}"
    current_year = str(now.year)
    
    return {
        "CD": {
            "TSK": schema_type,
            "LIR": "C",
            "NCT": {
                "NCTR": "12",  # Lazio
                "NCTN": nct_number
            },
            "ESC": "SSABAP-RM",
            "ECP": "SSABAP-RM"
        },
        "OG": {
            "OGT": {
                "OGTD": "",
                "OGTT": "",
                "OGTN": ""
            },
            "CLS": {
                "CLSC": "",
                "CLSS": "",
                "CLSP": ""
            }
        },
        "LC": {
            "PVC": {
                "PVCS": "Italia",
                "PVCR": "Lazio",
                "PVCP": "RM", 
                "PVCC": "Roma",
                "PVCL": ""
            },
            "PVL": {
                "PVLN": site_name,
                "PVLI": "Palatino",
                "PVLE": "Parco archeologico del Colosseo"
            },
            "LDC": {
                "LDCN": "",
                "LDCU": "",
                "LDCQ": "",
                "LDCS": ""
            }
        },
        "DT": {
            "DTZ": [{
                "DTZG": "età imperiale",
                "DTZS": "ca."
            }],
            "DTM": "dati stratigrafici"
        },
        "MT": {
            "MTC": {
                "MTCM": [],
                "MTCT": []
            },
            "MIS": {
                "MISA": None,
                "MISL": None,
                "MISP": None,
                "MISD": None,
                "MISE": None,
                "MISU": "cm"
            }
        },
        "DA": {
            "DES": {
                "DESO": ""
            },
            "ISR": [],
            "NSC": ""
        },
        "CO": {
            "STC": {
                "STCC": "buono",
                "STCS": ""
            }
        },
        "AD": {
            "ADSP": "1"
        },
        "CM": {
            "CMP": {
                "CMPD": current_year,
                "CMPN": ""
            },
            "RSR": "",
            "FUR": ""
        }
    }

# Funzione principale per compatibilità
def generate_default_iccd_data(schema_type: str, site_name: str = "Domus Flavia") -> Dict[str, Any]:
    """Genera dati ICCD di default - usa schema RA 3.00 ufficiale."""
    return generate_default_iccd_data_ra300(schema_type, site_name)